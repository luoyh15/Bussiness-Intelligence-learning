...
def group_statistic(df, group_cols):
    result = df
    # 树模型的统计特征更多
    agg_fun = [len, 'min', 'max', 'median', 'mean', 'std', 'sum', 'skew', pd.Series.kurt, 'mad']
    for col in group_cols:
        group = df[[col,'price']].groupby(col)
        agg = group.agg(agg_fun)
        agg.columns = [col+'_'+fun[1] for fun in agg.columns]
        result = result.merge(agg, how='left', on=col)
    return result
...
if __name__ == '__main__':
    ...
    # 需要统计特征的列（树模型统计特征更多）
    group_cols = ['model', 'brand', 'bodyType', 'fuelType', 'gearbox', 'notRepairedDamage', 'city', 'regionCode']
    # 进行特征统计
    data = group_statistic(data, group_cols)

    # 不需要的列
    drop_col = ['SaleID', 'name', 'regDate', 'creatDate', 'seller', 'offerType',  'price']
    # 类别列
    ...
    lgb_path = path + 'lgb/'
    train_X.to_pickle(lgb_path+'train_X.pkl')
    test_X.to_pickle(lgb_path+'test_X.pkl')
    train_y.to_pickle(lgb_path+'train_y.pkl')